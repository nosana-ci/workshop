...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--prompt",
        type=str,
        required=True,  # Changed from default=PROMPT
        help="The instance prompt."
    )
    parser.add_argument(
        "--max_sequence_length",
        type=int,
        default=77,  # Moved default here
        help="Maximum sequence length to use for computing the embeddings. The more the higher computational costs.",
    )
    parser.add_argument(
        "--local_data_dir",
        type=str,
        required=True,  # Changed from default=LOCAL_DATA_DIR
        help="Path to the directory containing instance images."
    )
    parser.add_argument(
        "--output_path",
        type=str,
        default="embeddings.parquet",  # Changed from default=OUTPUT_PATH
